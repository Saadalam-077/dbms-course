...
const AdminDashboard = ({ students, onNavigate, onLogout, onRefresh }) => {
  ...
  const labs = [
    { lab: 1, titleEn: "Introduction to DBMS" },
    { lab: 2, titleEn: "Relational Data Model" },
    { lab: 3, titleEn: "SQL Basics - DDL" },
    { lab: 4, titleEn: "SQL Queries - DML" },
    { lab: 5, titleEn: "Advanced SQL Queries" },
    { lab: 6, titleEn: "ER Diagrams" },
    { lab: 7, titleEn: "Normalization" },
    { lab: 8, titleEn: "Indexing & Performance" },
    { lab: 9, titleEn: "Transactions & ACID" },
    { lab: 10, titleEn: "Database Project" },
  ];
  ...
  const getGradeColor = (g) => {
    ...
    if (g === 'B+' || g === 'B') return 'text-orange-400';
    if (g === 'C+' || g === 'C') return 'text-yellow-400';
    if (g === 'D+' || g === 'D') return 'text-amber-400';
  ...
  const getGradeBg = (g) => {
    ...
    if (g === 'B+' || g === 'B') return 'bg-orange-500/20 border-orange-500/50';
    if (g === 'C+' || g === 'C') return 'bg-yellow-500/20 border-yellow-500/50';
    if (g === 'D+' || g === 'D') return 'bg-amber-500/20 border-amber-500/50';
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-orange-900/30 to-slate-900">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-orange-300/70 text-xs">DBMS | لوحة تحكم المدير</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-slate-800/50 rounded-2xl p-5 border border-slate-700">
            ...
            <p className="text-3xl font-bold text-orange-400">{stats.avgProgress}%</p>
          </div>
          <div className="bg-slate-800/50 rounded-2xl p-5 border border-slate-700">
        ...
        <div className="bg-slate-800/50 rounded-2xl border border-slate-700 overflow-hidden">
          ...
          {filteredStudents.length === 0 ? (
            <div className="p-12 text-center">
              ...
              <button onClick={handleRefresh} className="px-4 py-2 bg-orange-600 hover:bg-orange-500 text-white rounded-lg text-sm">Refresh Data</button>
            </div>
          ) : (
                  ...
                  {filteredStudents.map(student => {
                    ...
                    return (
                      <tr key={student.id || student.studentNumber} className="hover:bg-slate-700/30">
                        ...
                        <td className="px-4 py-3">
                          <span className="px-2 py-1 bg-slate-700 rounded text-orange-400 text-sm">{student.sectionNumber}</span>
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex items-center justify-center gap-2">
                            <div className="w-24 h-2 bg-slate-700 rounded-full overflow-hidden">
                              <div className="h-full bg-gradient-to-r from-orange-500 to-amber-400" style={{ width: `${pct}%` }}></div>
                            </div>
      ...
      <footer className="py-6 text-center border-t border-slate-800 mt-8">
        <p className="text-slate-500 text-sm">DBMS | Taif University</p>
      </footer>
    </div>
  );
};
