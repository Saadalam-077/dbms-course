...
const LoginPage = ({ onLogin, onNavigate }) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-orange-900/30 to-slate-900 flex items-center justify-center p-4">
      <div className="bg-slate-800/50 backdrop-blur rounded-2xl p-8 w-full max-w-md border border-slate-700">
        <div className="text-center mb-8">
          ...
          <p className="text-orange-300/70 font-arabic">نظام إدارة قواعد البيانات</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button type="submit" disabled={loading} className="w-full py-3 bg-orange-600 hover:bg-orange-500 text-white font-bold rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
            {loading ? (
              <>
        ...
        <p className="text-center text-slate-400 mt-6">
          ...
          <button onClick={() => onNavigate('register')} className="text-orange-400 hover:underline">Register</button>
        </p>
      </div>
    </div>
  );
