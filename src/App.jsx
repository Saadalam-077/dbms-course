...
function App() {
  ...
  useEffect(() => {
    loadStudents();
    const saved = localStorage.getItem('dbmsUser');
    if (saved) {
  ...
  useEffect(() => {
    if (user) {
      localStorage.setItem('dbmsUser', JSON.stringify(user));
    } else {
      localStorage.removeItem('dbmsUser');
    }
  }, [user]);
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-orange-900/30 to-slate-900 flex items-center justify-center">
